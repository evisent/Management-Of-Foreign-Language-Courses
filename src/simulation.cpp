...
void Simulator::rand_leave(GroupManager& manager, std::vector<std::unique_ptr<Student>>& students, std::vector<std::unique_ptr<Student>>& individual_students){
    ...
    for (int i = 0; i < students.size(); ++i){
        int chance2 = my_rand() % 100;
        int level = students[i]->get_languages()[0]->get_level();
        int total = 11;
        std::cout << chance2 << '\n';
        if (chance2 < total - level * 3){       // 8%, 5%, 2%
            manager.delete_student(*students[i], students[i]->get_languages()[0]->get_name());
            index.push_back(i);
        }  
    }

    for (int i = 0; i < index.size(); ++i){
        students.erase(students.begin() + index[i]);
    }
